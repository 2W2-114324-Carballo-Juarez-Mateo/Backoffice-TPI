...
- **Multitenancy (TenantContext + RLS):** read models acotados por `course_id` (tenant = curso-cohorte); `TenantContext` setea `app.current_course` desde el contexto validado; **RLS** refuerza el aislamiento a nivel de base (no reemplaza la autorización). **ADMIN** con alcance puntual (un curso) o **global** (`app.current_course='ALL'`, panel y comparativas — autorizado por rol, sin `BYPASSRLS`).
...
## 9. Pruebas
Unitarias (alcance, agregación) · integración (MockMvc + Testcontainers, matrícula T02 mockeada) · **multitenancy obligatorio: PROFESOR A → cohorte A → 200; PROFESOR A → cohorte B → 403** (+ verificación de RLS).
...
> Backlog completo: ver `docs/10-planificacion.md`.