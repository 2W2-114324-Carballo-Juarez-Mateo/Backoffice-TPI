...
| **TOTAL** | | | | **6** | **420,0** | **76,5** | **343,5** | | **310,7** |

> **Julieta (Dev 5) se cambió de grupo (2026-09):** se retiró del microservicio; su capacidad (51,5 h) se descuenta. **Capacidad real del equipo: 310,7 h** (9 devs activos). Sus tareas hechas (US-08 T1, G4) quedan; las pendientes se reasignaron (ver §6).
...
**241 h sobre 310,7 h = 77,6 % de uso** (9 devs activos; Julieta se fue — sus 8 h hechas quedan). Queda ~70 h de margen para lo que no vimos venir.
...
| FE-4b | Pantalla de administración: estado 2FA/sesión | FRONTEND | Regina | 3 |
...
| Testcontainers | **US-02 T5** (Máximo) y **US-01 T8** (Mateo) — ya estaban dentro |
...
## 6 · Resumen de carga (9 devs activos · los 9 cubren las 5 capas)

> **Julieta (Dev 5) se cambió de grupo.** Quedan como hechas por ella: **US-08 T1** (PR #9, falta merge) y **G4** (mergeada). Sus tareas pendientes se **reasignaron** (US-02 T2 → Mateo, hecha · US-08 T4 → Valentina · FE-4b → Regina · US-02 T6 → Máximo · US-01 T10b → Luciano).

| Dev | Integrante | Rol | Capacidad | Horas | % uso | Código (BACK+FRONT) | Capas |
|---|---|---:|---:|---:|---:|---|
| **1** | Paz, Luciano | MSII+PIV | 39,4 | 30 | 76,1 % | 19 h | BACK + FRONT + TEST + REV + DOC (camino crítico) |
| **2** | Carballo Juarez, Mateo | MSII+PIV | 41,5 | 31 | 74,7 % | 14 h | BACK + FRONT + TEST + REV + DOC |
...
| **6** | Maldonado, Valentina | MSII+PIV | 35,3 | 31 | 87,8 % | 17 h | BACK + FRONT + TEST + REV + DOC |
| **7** | Cerquatti, Máximo | MSII+PIV | 46,4 | 32 | 69,0 % | 17 h | BACK + FRONT + TEST + REV + DOC |
| **8** | Cerasulo, Regina Loreta | MSII+PIV | 35,3 | 22 | 62,3 % | 10 h | BACK + FRONT + TEST + REV + DOC |
...
**Total activo: 233 h / 310,7 h (9 devs) = 75 %.** + 8 h completadas por Julieta (US-08 T1, G4) = **241 h** del sprint. Horas ~62–88 % (dentro de capacidad; Valentina roza el 88 % por ser dueña de US-08) · código ~10–19 h · **los 9 devs cubren las 5 capas (BACK + FRONT + TEST + REV + DOC)**; Ana Paula solo DOC (MSII).
...
| T2 | Adoptar el `EventEnvelope<T>` oficial y mapear `PARAMETER_CHANGED` / `GLOBAL_CONFIGURATION_CHANGED` | BACKEND | Mateo (hecho) | 6 |
...
| T6 | Validar resiliencia ante caída del broker | TEST | Máximo | 6 |
...
| T10b | Peer review del **versionado/no retroactividad** de US-01 (revista trabajo de otro) | REVISION | Luciano | 1 |
...
| # | Tarea | Rol | Dev | Horas |
|---|---|---|---|---:|
| T1 | Migración y tabla de deduplicación `processed_event` | BACKEND | Julieta (fuera) — hecha, PR #9 | 4 |
...
| T4 | Dead Letter Topic para eventos malformados | BACKEND | Valentina | 4 |
...
| G4 | **Registro formal en `CONTRATOS.md`** — estado, fecha de acuerdo, responsable de cada lado, versión y evidencia | DOCUMENTACION | Julieta (fuera) — hecha | 4 |
